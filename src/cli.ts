...
program
  ...
  .action(async () => {
    ...
    const { framework, language, usePrisma, runNpmInstall } = await inquirer.prompt([
      {
        type: 'list',
        name: 'framework',
    ...
    if (usePrisma) {
      ...
      if (!fs.existsSync(prismaTemplatePath)) {
        console.error(chalk.red(`Prisma template path does not exist: ${prismaTemplatePath}`))
        process.exit(1)
      }
      ...
      try {
        ...
        console.error(chalk.red(`Error copying Prisma files: ${error.message}`))
        process.exit(1)
      }
